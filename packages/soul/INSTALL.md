...
prompt loader inlines it at the very top of your system prompt on every build,
so your character is always in context with no per-session read. It depends on
the `memory` package: `Soul.md` lives in that store (`data/memory/`) and is
...
3. Read `data/memory/Soul.md` to see the seeded starter, then make it yours over
   time — see the skill for when and how. No prompt wiring to do: the loader
   picks up `Soul.md` automatically, and re-reads it each build, so every edit
