...
# ruff: noqa
# mypy: ignore-errors
# Vendored from NousResearch/hermes-agent (MIT); adapted for chief.
"""Watch GitHub activity — issues, pulls, releases, or commits — with dedup.

Run from chief's repo root via the shell tool (usually from a `schedule`
prompt):

    uv run python skills/watchers/scripts/watch_github.py --name <name> ...

First run records a baseline (emits nothing); later runs emit only new
items. Empty stdout means nothing new.

Set GITHUB_TOKEN (or GH_TOKEN) in the environment to avoid
the 60 req/hr anonymous rate limit.
"""
