"""The agent backend: constructs the per-task session the engine drives.

:class:`CopilotBackend` is chief's sole harness (#88 removed the claude-agent-sdk
backend and the ``AgentBackend`` protocol / ``select_backend`` registry — the engine's
real seam is the ``SessionFactory`` callable, which every test fakes). It builds a
per-task :class:`~chief.core.session.SessionProto` over the **GitHub Copilot SDK**,
wiring the model, the resume pointer, chief's permission callback + hooks (adapted onto
the Copilot surface by :mod:`chief.core.copilot_gate`), the in-process tools + MCP
servers, the persona, the BYOK provider, and the category-routed subagents / M10 skill
dirs. The returned session runs the streaming turn (``run_turn``) and switches the model
(``set_model``).
"""
...
from .session import SessionProto


class CopilotBackend:
    """Builds a per-task session over the GitHub Copilot SDK (#76, part of #72).
    ...
    **Concurrency model (#80).** One Copilot runtime per chat: every
    ...
    its own ``client_factory()`` → its own CLI-server subprocess. chief's per-chat
    sessions stay isolated with no shared-client locking — the SDK's ability to run N
    sessions on one client is deliberately unused, keeping ``aclose`` a per-session
    teardown. ``TaskManager``'s ``Semaphore`` (which bounds concurrently *generating*
    turns) is orthogonal and fits this model unchanged.
    ...
    * ``plugins`` / ``skills`` — chief's ``[{"type":"local","path":…}]`` + ``list[str]``
      shape was claude-agent-sdk's plugin-manifest form; the Copilot equivalent is
