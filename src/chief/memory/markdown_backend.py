...
distill/forget/scaffold ops can't interleave file + commit. ``purge_expired`` drops
TTL-expired facts (DESIGN: flag time-sensitive facts). The ``facts/`` listing is
auto-generated by scanning the directory at prompt-build time — no stored index file.
"""
