...
forget/scaffold ops can't interleave file writes. The shared :class:`GitVersioner`
instance self-serializes its own git subprocess calls via an internal lock, so
concurrent callers (turn loop + store mutators) never collide on git's ``index.lock``
(#29). ``purge_expired`` drops TTL-expired facts (DESIGN: flag time-sensitive facts).
The ``facts/`` listing is auto-generated by scanning the directory at prompt-build time
— no stored index file.
"""
