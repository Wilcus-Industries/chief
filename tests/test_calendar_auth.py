...
exercise path handling, scope wiring, the authorized-user token write, and the CLI's
exit codes / operator instructions.
"""
...
def _fake_consent(captured: dict[str, object]) -> auth.ConsentFn:
    ...
    def consent(client_secrets: Path, scopes: list[str]) -> auth._Credentials:
        ...
        return _FakeCredentials('{"type": "authorized_user", "refresh_token": "rt"}')

    return consent


def test_mint_token_writes_authorized_user_json(tmp_path: Path) -> None:
...
def test_main_success_writes_and_instructs(
    ...
    assert code == 0
    assert out.exists()
    assert "google_calendar_token" in capsys.readouterr().out
